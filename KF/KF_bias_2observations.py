...
z_{k+1, 2} =  H_{k, 2} x_{k+1} + v_{k+1, 2} ~ R_{k+1, 2}
"""
'''
centralized  KF is:
y = [x.T b.T].T
...
observations_2 = np.empty((observation_sz, 0))
estimated_state_array = np.empty((state_sz+bias_sz, 0))
bias_ground_true = np.empty((bias_sz, 0))
...
b = np.zeros((bias_sz, 1))
# b = 0.1*np.ones((2, 1))
P_x = 1.0 * np.eye(state_sz)
P_b = 1.2 * np.eye(bias_sz)

A = np.eye(state_sz)
A[0, 2] = dt
A[1, 3] = dt
# B = 0.125*np.eye(state_sz, bias_sz)
B = np.zeros((state_sz, bias_sz))
Q_x = 0.25 * np.eye(state_sz)
Q_b = 1.0 * np.eye(bias_sz)

y = np.zeros((observation_sz, 1))
H_1 = np.eye(observation_sz, state_sz)
H_2 = np.eye(observation_sz, state_sz)
C = np.eye(observation_sz, bias_sz)
...
"""
z = np.vstack((x, b))
F = np.vstack((np.hstack((A, B)), np.hstack(((np.zeros((bias_sz, state_sz)), np.eye(bias_sz))))))
Q = np.eye((state_sz + bias_sz))
Q[0:state_sz, 0:state_sz] = Q_x
Q[state_sz:state_sz+bias_sz, state_sz:state_sz+bias_sz] = Q_b
L_1 = np.hstack((H_1, C))
L_2 = np.hstack((H_2, np.zeros((bias_sz, bias_sz))))
P = np.vstack((np.hstack((P_x, np.zeros((P_x.shape[0], b.shape[0])))), np.hstack((np.zeros((b.shape[0], P_x.shape[0])), P_b))))
I = np.eye(state_sz + bias_sz)
...
plt.plot(points_2[0], points_2[1], color='green')
plt.subplot(2, 2, 4)
...
for i in np.arange(0, N_iter):
    ...
    plt.scatter(observations_2[0, -1], observations_2[1, -1], c='g')
...
plt.plot(ts, bias_ground_true[0], linewidth=4)
plt.subplot(2, 1, 2)
plt.plot(ts, estimated_state_array[5])
plt.plot(ts, bias_ground_true[1], linewidth=4)
plt.show()