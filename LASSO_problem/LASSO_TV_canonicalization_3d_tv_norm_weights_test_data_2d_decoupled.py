...
path2save = '/home/morzh/work/DSOPP/test/test_data/lasso_tv_problem_2d_decoupled'
number_data_samples = 300
dimensions = 2
np.random.seed(19)

for idx in range(number_data_samples):
    elements_number = np.random.randint(dimensions, 2000)
    # elements_number = 5
    ...
    weights = 0.1 + np.random.random((dimensions, elements_number))
    # weights = np.ones((dimensions, elements_number))
    ...
    functions = [0.5 * parameters[2] * cp.sum_squares(beta - y),
                 ...
                 parameters[1] * cp.tv(beta[0]) + parameters[1] * cp.tv(beta[1])]
