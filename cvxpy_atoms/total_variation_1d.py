    ...
    # L2 norm for matrices.
        ...
        for mat in values:
            diffs += [
                ...
                mat[1:rows, :] - mat[0:rows-1, :],
                # mat[1:rows, 1] - mat[0:rows-1, 1]
            ]
        length = diffs[0].shape[0]*diffs[0].shape[1]
        stacked = vstack([reshape(diff, (1, length)) for diff in diffs])
        return sum(norm(stacked, p=2, axis=0))


def tv_norm_rows(value, *args):
    """Total variation of a vector, matrix, or list of matrices.
    ...
    # L2 norm for matrices.
        ...
        for mat in values:
            diffs += [
                norm(mat[1:rows, :] - mat[0:rows-1, :], axis=1)
            ]
        length = diffs[0].shape[0]
        stacked = vstack([reshape(diff, (1, length)) for diff in diffs])


def tv_norm_cols(value, *args):
    """Total variation of a vector, matrix, or list of matrices.
    ...
    # L2 norm for matrices.
        ...
        for mat in values:
            diffs += [
                norm(mat[:, 1:cols] - mat[:, 0:cols-1], axis=0)
            ]
        length = diffs[0].shape[0]
        stacked = vstack([reshape(diff, (1, length)) for diff in diffs])
        return sum(norm(stacked, p=2, axis=0))
