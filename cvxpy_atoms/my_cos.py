...
def my_cos(expr):
    ...
    return atom_my_cos(expr)

class atom_my_cos(Atom):

    _allow_complex = False
    def __init__(self, x):
        super(atom_my_cos, self).__init__(x)
    ...
    def validate_arguments(self):
        ...
        if not self.args[0].is_scalar():
            raise ValueError("Argument of my_cos must be a scalar.")
        super(atom_my_cos, self).validate_arguments()
