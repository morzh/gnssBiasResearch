...
gps_poses_number = aligned_pc_1.shape[1]
bias_smoothed = get_smoothed_piecewise_bias(number_iterations=gps_poses_number, jumps_number=(1, 10), mult=0.05, smooth_factor=(25, 15), savgol_params=(35, 3))
aligned_pc_2 = aligned_pc_1 + bias_smoothed
...
def func_optimize(params, pc_1, pc_2, z, w, rho):
    ...
    e2 = bias - z + w
    cost = np.sum(e.flatten()**2) + rho * np.sum(e2.flatten()**2)
    print(cost)
    return cost
...
for idx in range(901):
    ...
    step = 0.9
    z = proxL1Norm(z + rho * step * (b_reg - z + w), step*lmbda_2)
    w += b_reg - z
