...
path_DSO = os.path.join(path, 'track04')
path_CARLA = os.path.join(path, 'track04')
...
for idx in range(701):
    ...
    funcs = [0.5*rho * cp.sum_squares(b_reg - z_reg + w), lmbda_2 * cp.sum(cp.norm1(z_reg, axis=0))] #, lmbda * tv_norm_cols(z_reg)
