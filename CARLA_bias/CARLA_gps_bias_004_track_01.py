...
path_DSO = '/media/morzh/ext4_volume/work/hdmapper_data/track03'
path_CARLA = '/media/morzh/ext4_volume/work/hdmapper_data/track03'
...
gps_accuracy = associated_track.getGPSAccuracyStacked()
...
if show_corresps:
    for idx in range(aligned_pc_1.shape[1]):
        points = np.hstack((aligned_pc_1[:, idx].reshape(3, 1), aligned_pc_2[:, idx].reshape(3, 1)))
...
def func_optimize(params, pc_1, pc_2, z, w, rho, accuracy):
    sim3_grp = sp.SIM3()
    ...
    e /= accuracy
    e2 = bias /accuracy - z + w
    # e2 /= accuracy
    return 0.5*np.sum(e.flatten()**2) + 0.5*rho * np.sum(e2.flatten()**2)


for idx in range(501):
    least_squares = minimize(func_optimize, init_guess, args=(aligned_pc_1, aligned_pc_2, z, w, rho, gps_accuracy), options={'maxiter': 8})

    b_reg = least_squares.x_1[grp_params_num:].reshape(dims_num, -1) / gps_accuracy
