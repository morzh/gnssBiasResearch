...
module and show how to use various LASSO_problem that are included in the
PyLops library.
...
# be solved using explicit matrix LASSO_problem as well as iterative LASSO_problem. In
...
# (without regularization term for now) and customize our LASSO_problem using
# ``kwargs``.
xinv = \
...
# problems.
#
# We have however so far only considered LASSO_problem that can include additional
# norm-2 regularization terms. A very active area of research is that of
# *sparsity-promoting* LASSO_problem (also sometimes referred to as *compressive
...
# :py:class:`pylops.optimization.sparsity.FISTA` LASSO_problem to estimate our input
# signal.
